...
namespace FrostFlow.Controllers
{
    ...
    public class HomeController(IUsuarioModel _usuarioModel, IUtilitariosModel _utilitariosModel) : Controller //Inyección de dependencia
    {
        ...
        public IActionResult Login(Usuario entidad)
        {
            ...
            resp.Dato.contrasenna = _utilitariosModel.Decrypt(entidad.contrasenna);

            if (resp.Dato.contrasennaTemporal == true)
            {
            ...
            else
            {
                //entidad.contrasenna = _utilitariosModel.Encrypt(entidad.contrasenna);
                    if (resp.Codigo == "1")
                    {
                        HttpContext.Session.SetString("id_Usuario", resp.Dato.id_Usuario.ToString());
